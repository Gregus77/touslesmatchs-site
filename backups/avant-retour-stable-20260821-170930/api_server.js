...
function sendToPaidChannels(text, opts = {}) {
  ...
  push(TELEGRAM_PREMIUM_CHANNEL_ID, "intensif");
...
Réponds en JSON pur (pas de markdown):
{
  ...
  try {
    ...
    if (!chiefProviders.length && OPENROUTER_API_KEY
        && analysisEngine.allowOfficialOpenRouterFallback(db, { agentLabel: "Chief", matchKey: `${match.home || "?"}_${match.away || "?"}`, competition: match.competition || match.league || "", modelKey: "kimi" })) {
      chiefProviders.push({ kind: "openai", url: "https://openrouter.ai/api/v1/chat/completions", key: OPENROUTER_API_KEY, model: process.env.OR_KIMI_MODEL || "moonshotai/kimi-k3" });
    }
  ...
  if (!_blockReason) {
    ...
    if (!_signalSentCache.has(signalKey)) {
      ...
      const gradeIntensif = gradeStandard || (diffusable && minuteOk && voteCountForSignal >= 4 && conf >= TH.premium);
      const gradeElite = false; // ancien palier désactivé
      shadowWorthy = gradeIntensif;
      ...
      // 🟣 INTENSIF — cap 10/j
      if (TELEGRAM_PREMIUM_CHANNEL_ID && gradeIntensif && _premiumSignalDaily.count < PREMIUM_SIGNAL_DAILY_CAP) {
        _premiumSignalDaily.count++;
        sendTelegramMessage(TELEGRAM_PREMIUM_CHANNEL_ID, tgPremium + tierTag("🟣 INTENSIF")).then(ok => {
          if (ok) markSignalSent(match.home, match.away, "sig_sent_premium", _ligneAnalysee);
          else _premiumSignalDaily.count--;
          console.log(`[signal-fort] Telegram intensif (${_premiumSignalDaily.count}/${PREMIUM_SIGNAL_DAILY_CAP}) conf=${conf} cote=${realOdd}: ${ok ? "OK" : "FAIL — non marque car envoi Telegram KO, quota rendu"}`);
        });
      } else if (TELEGRAM_PREMIUM_CHANNEL_ID && gradeIntensif) {
        console.log(`[signal-fort] Intensif: plafond ${PREMIUM_SIGNAL_DAILY_CAP}/jour atteint, skip`);
      }

      // 🟠 ELITE/VIP — cap 30/j, multisport, alertes prioritaires
      if (false && eliteDistinct && gradeElite && _eliteSignalDaily.count < ELITE_SIGNAL_DAILY_CAP) {
...
function markSignalSent(home, away, col, matchKey) {
  ...
  try {
    ...
    if (!r.changes) console.warn(`[signal-sent] ${col} NON marque pour ${home} vs ${away}`);
...
app.get("/analysis-history", (req, res) => {
  // [analysis-history] compat Live Totaux : protège les anciennes PWA.
  try {
    const limit=Math.min(Math.max(Number(req.query.limit)||40,1),100);
    const raw=db.prepare(`
      SELECT home, away, competition, best_bet, outcome, real_odd, analysed_at,
             consensus_votes
      FROM concile_analyses
      WHERE analysed_at >= '2026-08-21 00:00:00'
        AND outcome IN ('win','loss')
        AND best_bet IN ('Over 2.5 buts','Under 2.5 buts')
        AND (sig_sent_standard=1 OR sig_sent_premium=1)
      ORDER BY analysed_at DESC
    `).all();
    const seen=new Set(), analyses=[];
    for(const r of raw){
      const k=String(r.home||"").toLowerCase()+"|"+String(r.away||"").toLowerCase()+"|"+String(r.analysed_at||"").slice(0,10);
      if(seen.has(k))continue;
      seen.add(k); analyses.push(r);
      if(analyses.length>=limit)break;
    }
    return res.json({ok:true,total:analyses.length,analyses});
  } catch(e) {
    console.error("[analysis-history] compat Live Totaux:",e.message);
    return res.status(500).json({ok:false,analyses:[]});
  }
