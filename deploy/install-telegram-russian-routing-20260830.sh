...
const_anchor = 'const TELEGRAM_ELITE_CHANNEL_ID = process.env.TELEGRAM_ELITE_CHANNEL_ID || TELEGRAM_PREMIUM_CHANNEL_ID || "";\n'
const_block = const_anchor + '''const TELEGRAM_RU_FREE_CHANNEL_ID = process.env.TELEGRAM_RU_FREE_CHANNEL_ID || "";
...
if "const TELEGRAM_RU_FREE_CHANNEL_ID =" not in server:
    if const_anchor not in server:
        raise SystemExit("BLOQUE: constantes Telegram introuvables")
    server = server.replace(const_anchor, const_block, 1)
