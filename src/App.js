...
export default function App() {
  ...
  var header = React.createElement("header", {style:{borderBottom:"1px solid rgba(212,175,55,0.2)",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(8,12,20,0.98)",position:"sticky",top:0,zIndex:50,flexWrap:"wrap",gap:"8px",maxWidth:"100%",overflowX:"hidden"}},
    ...
    React.createElement("nav", {className:"main-nav",style:{display:"flex",gap:"6px",alignItems:"center",flexWrap:"wrap",flex:1,justifyContent:"flex-end"}},
      ["home","preuves","bookmakers","calculateur","analyse","live-ia"].map(function(p){
        var labels = {home:t("nav_choix"), preuves:t("nav_preuves"), bookmakers:t("nav_bookmakers"), calculateur:"💰 Calculateur", analyse:t("nav_analyse"), "live-ia":"🔮 Live IA"};
        var isAnalyse = p==="analyse";
        var isCalc = p==="calculateur";
        var isLiveIA = p==="live-ia";
        return React.createElement("button", {key:p, onClick:function(){setPage(p);}, style:{background:page===p?(isLiveIA?"rgba(201,162,39,0.25)":(isAnalyse?"rgba(201,162,39,0.2)":"rgba(212,175,55,0.15)")):"transparent",border:"1px solid "+(page===p?"#d4af37":(isLiveIA?"rgba(201,162,39,0.5)":(isAnalyse||isCalc?"rgba(201,162,39,0.3)":"rgba(255,255,255,0.1)"))),color:page===p?"#d4af37":(isLiveIA?"#C9A227":(isAnalyse||isCalc?"#C9A227":"#666")),padding:"6px 14px",borderRadius:"4px",cursor:"pointer",fontSize:"12px",fontWeight:(isLiveIA||isAnalyse||isCalc)?"600":"400"}}, labels[p]);
      }),
  ...
  if(page==="live-ia") return React.createElement(React.Fragment, null, header, React.createElement(LiveIA, null), footer, bandeauLegal);
