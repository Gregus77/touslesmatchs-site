...
(async()=>{
  ...
  now=Date.parse('2026-09-19T22:02:00Z');await context.recoverProviderDailyQuota();assert.equal(requests,3);assert.equal(deletes,0);
  await context.recoverProviderDailyQuota();assert.equal(requests,3);
  ...
  console.log('PASS Paris 00:00 and 00:02 probes, exhausted quota preserved, natural recovery and throttling');
