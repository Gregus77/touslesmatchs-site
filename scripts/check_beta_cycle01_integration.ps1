...
$checks = @{
  ...
  (Join-Path $root 'scripts\api_server.js') = @('beta_plus05_applications', 'app.get("/beta-plus05/status"', 'app.post("/beta-plus05/apply"', 'app.get("/admin/beta-plus05/applications"', 'formatBetaApplicationsCsv', 'buildBetaPlus05InvitationEmail', 'TELEGRAM_GOAL05_INVITE_URL', 'inviteEmailQueued', 'BETA_PLUS05_CAPACITY');
