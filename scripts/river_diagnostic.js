...
function inject(source){if(source.includes('TLM-RIVER-DIAGNOSTIC-20260920'))return source;if(!source.includes('</body>'))throw Error('BODY_ANCHOR_MISSING');return source.replace('</body>',`<!-- TLM-RIVER-DIAGNOSTIC-20260920 --><script>if(new URLSearchParams(location.search).get('test')==='river'){var panel=document.createElement('section');panel.id='river-diagnostic';panel.style.cssText='display:block;position:relative;z-index:10;background:#0b1024;color:#eaf4ff;padding:20px;max-width:1200px;margin:85px auto 16px;border:2px solid #49def1;border-radius:16px;font:16px system-ui';panel.textContent='Chargement du test exceptionnel…';document.body.prepend(panel);fetch('/river-diagnostic-20260920.html',{cache:'no-store'}).then(function(r){if(!r.ok)throw Error();return r.text();}).then(function(text){var report=new DOMParser().parseFromString(text,'text/html');panel.textContent='';report.querySelectorAll('h2,p,li').forEach(function(source){var el=document.createElement(source.tagName==='H2'?'h2':'p');el.textContent=source.textContent;el.style.cssText='margin:10px 0;color:#eaf4ff';panel.append(el);});}).catch(function(){panel.textContent='Rapport de test indisponible.';});}</script></body>`);}
async function run(){
