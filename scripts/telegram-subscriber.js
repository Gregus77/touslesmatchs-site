...
class InviteLinkManager {
  ...
  _cacheKey(chatId, memberLimit) {
    return `${chatId}:${memberLimit}`;
  }

  async getOrCreate(chatId, opts = {}) {
    const memberLimit = opts.memberLimit !== undefined ? opts.memberLimit : this.memberLimit;

    if (memberLimit === 1) {
      return this._createLink(chatId, memberLimit);
    }

    const key = this._cacheKey(chatId, memberLimit);
    ...
    const promise = this._createLink(chatId, memberLimit);
  ...
  async _createLink(chatId, memberLimit) {
    const limit = memberLimit !== undefined ? memberLimit : this.memberLimit;
    ...
    if (link && limit !== 1) {
      const key = this._cacheKey(chatId, limit);
      this._cache.set(key, {
        link,
