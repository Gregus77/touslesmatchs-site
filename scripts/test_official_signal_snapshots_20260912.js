...
const matchScope = model.fixtureScope(match,testDay);
...
model.capture(db,{id:'42_day_15_0-0',fixtureScope:matchScope,match,analysisMatchKey:'42_day',minute:20,scoreHome:0,scoreAway:0,redCardsHome:0,redCardsAway:0,votes:seats(['under','under','over',null,'under']),consensus:'under',consensusVotes:3,confidence:81,realOdd:1.7,ruleVersion:'legacy-v1',createdAt:'2026-09-12T20:00:00Z'});
...
model.capture(db,{id:'42_day_30_0-1',fixtureScope:matchScope,match:goalMatch,analysisMatchKey:'42_day',minute:39,scoreHome:0,scoreAway:1,redCardsHome:0,redCardsAway:0,votes:seats(['over','over','over',null,'over']),consensus:'over',consensusVotes:4,confidence:82,realOdd:1.91,ruleVersion:'v2',createdAt:'2026-09-12T20:03:00Z'});
...
assert.equal(db.prepare('SELECT COUNT(*) n FROM official_vote_snapshots').get().n,5);
assert.equal(db.prepare('SELECT COUNT(*) n FROM official_signal_registry').get().n,1);
