...
(async()=>{
 ...
 let voteStorage={predictions:0,ou25_predictions:0,distinct_keys:0,market_predictions:0,market_ou25:0,market_distinct_keys:0,analyses:0,max_consensus:0,four_plus:0};
 if(tables.has("agent_predictions")){const cols=new Set(db.prepare("PRAGMA table_info(agent_predictions)").all().map(x=>x.name));const tc=cols.has("created_at")?"created_at":null;if(tc){const row=db.prepare("SELECT COUNT(*) predictions, COUNT(DISTINCT match_key) distinct_keys, SUM(CASE WHEN lower(bet) LIKE '%over 2.5%' OR lower(bet) LIKE '%under 2.5%' THEN 1 ELSE 0 END) ou25 FROM agent_predictions WHERE datetime(created_at)>=datetime('now','-30 minutes')").get();voteStorage.predictions=Number(row.predictions)||0;voteStorage.ou25_predictions=Number(row.ou25)||0;voteStorage.distinct_keys=Number(row.distinct_keys)||0;}}
 if(tables.has("agent_market_predictions")){const cols=new Set(db.prepare("PRAGMA table_info(agent_market_predictions)").all().map(x=>x.name));const tc=cols.has("created_at")?"created_at":null;if(tc){const row=db.prepare("SELECT COUNT(*) predictions, COUNT(DISTINCT match_key) distinct_keys, SUM(CASE WHEN market_line='buts' AND (lower(bet) LIKE '%over 2.5%' OR lower(bet) LIKE '%under 2.5%') THEN 1 ELSE 0 END) ou25 FROM agent_market_predictions WHERE datetime(created_at)>=datetime('now','-30 minutes')").get();voteStorage.market_predictions=Number(row.predictions)||0;voteStorage.market_ou25=Number(row.ou25)||0;voteStorage.market_distinct_keys=Number(row.distinct_keys)||0;}}
 if(tables.has("concile_analyses")){const cols=new Set(db.prepare("PRAGMA table_info(concile_analyses)").all().map(x=>x.name));const tc=cols.has("analysed_at")?"analysed_at":cols.has("created_at")?"created_at":null;if(tc){const row=db.prepare("SELECT COUNT(*) analyses, COALESCE(MAX(consensus_votes),0) max_consensus, SUM(CASE WHEN consensus_votes>=4 THEN 1 ELSE 0 END) four_plus FROM concile_analyses WHERE datetime("+tc+")>=datetime('now','-30 minutes')").get();voteStorage.analyses=Number(row.analyses)||0;voteStorage.max_consensus=Number(row.max_consensus)||0;voteStorage.four_plus=Number(row.four_plus)||0;const detail=db.prepare("SELECT COALESCE(MAX(confidence),0) max_confidence, SUM(CASE WHEN real_odd BETWEEN 1.30 AND 2.10 AND lower(COALESCE(real_odd_source,'')) NOT LIKE '%estimation%' THEN 1 ELSE 0 END) real_odd_ok, SUM(CASE WHEN COALESCE(sig_sent_free,0)+COALESCE(sig_sent_standard,0)+COALESCE(sig_sent_premium,0)+COALESCE(sig_sent_elite,0)>0 THEN 1 ELSE 0 END) marked_sent, SUM(CASE WHEN diffusion_block IS NULL OR trim(diffusion_block)='' THEN 1 ELSE 0 END) unblocked, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%recovery%' THEN 1 ELSE 0 END) recovery_block, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%championnat hors%' THEN 1 ELSE 0 END) recovery_league, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%historique%' OR lower(COALESCE(diffusion_block,'')) LIKE '%domicile%' THEN 1 ELSE 0 END) recovery_history, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%absence%' THEN 1 ELSE 0 END) recovery_injuries, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%indicateur%' THEN 1 ELSE 0 END) recovery_indicators, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%moyenne%' OR lower(COALESCE(diffusion_block,'')) LIKE '%seuil%' THEN 1 ELSE 0 END) recovery_average, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%confiance%' OR lower(COALESCE(diffusion_block,'')) LIKE '%confidence%' THEN 1 ELSE 0 END) confidence_block, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%vote%' OR lower(COALESCE(diffusion_block,'')) LIKE '%siege%' THEN 1 ELSE 0 END) vote_block, SUM(CASE WHEN lower(COALESCE(diffusion_block,'')) LIKE '%cote%' OR lower(COALESCE(diffusion_block,'')) LIKE '%bookmaker%' THEN 1 ELSE 0 END) odds_block FROM concile_analyses WHERE datetime("+tc+")>=datetime('now','-30 minutes')").get();Object.assign(voteStorage,Object.fromEntries(Object.entries(detail).map(([k,v])=>[k,Number(v)||0])));}}
