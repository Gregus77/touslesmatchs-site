...
const checks = [
  ...
  ["quatre réponses suffisent", "const enoughOu25SeatsPresent = Number(voteInfo.vote_active || 0) >= CLIENT_OU25_MIN_VOTES;"],
  ["cote minimum 1.30", "const TIER_MIN_REAL_ODD = Math.max(1.30"],
  ...
  ["complément saison précédente", "const previousRows = await fetchSeasonRows(numericSeason - 1);"],
  ["trois indicateurs réellement appliqués", "const ok = combinedAligned && liveAligned"],
